...
class SemiMarkovConditionalRandomField(torch.nn.Module):
    ...
    outside_span_tag: int, optional.
        Integer ID for the NOT-A-SPAN tag.
    loss_type: str, default = "logloss".
        Other options are recall-oriented cost, or hamming cost.
    ...
    def _input_likelihood(self,
        ...
        """
        Computes the (batch_size,) partition function for the log-likelihood,
        which is the sum of the scores of all possible segmentations.
    ...
    def _joint_likelihood(self,
        ...
        for j in range(sequence_length):
            ...
            for d in range(min(self.max_span_width, sequence_length)):
                ...
                emit_score = logits[j][d].gather(
                        dim=1, index=current_tag).squeeze(1) * valid_tag_mask * mask[j]
    ...
    def forward(self,
        ...
        if self.loss_type == "roc":
            ...
            cost = torch.zeros(inputs.size())
        else:
            raise ConfigurationError(
                    "Invalid loss: {}. Try `roc`, `hamming` or `logloss`".format(self.loss_type))
    ...
    @staticmethod
    def convert_spans_into_sequence_of_tags(viterbi_spans: Dict[Tuple[int, int], int],
                                            sequence_length: int,
    ...
    def _get_recall_oriented_cost(self, tags: torch.Tensor):
        ...
        irrelevant_tags = tags.eq(self.default_tag) | tags.eq(self.outside_span_tag)
        irrelevant_tags_mask = 1-irrelevant_tags.float()
        false_negatives = - self.false_negative_penalty * cost * irrelevant_tags_mask
